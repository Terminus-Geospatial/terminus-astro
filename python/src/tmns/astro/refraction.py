...
def get_coefficients( pressure_pHa : float,
                      temp_c : float,
                      rel_humidity : float,
                      wavelength_micrometers : float ) -> tuple:
    '''
    Based on SOFA's iauRefco,
    - File:  refco.c
    ...
    optic = wavelength_micrometers <= 100.0

    # Restrict parameters to safe values. */
    temp_c       = max( min( temp_c, 200.0 ), -150.0 )
    pressure_pHa = max( min( pressure_pHa, 10000.0 ), 0.0 )
    rel_humidity = max( min( rel_humidity, 1.0 ), 0.0 )
    wavelength_micrometers = max( min( wavelength_micrometers, 1e6 ), 0.1 )

    # Water vapour pressure at the observer. */
    if pressure_pHa > 0.0:
        ps = math.pow( 10.0, ( 0.7859 + 0.03477 * temp_c ) / ( 1.0 + 0.00412 * temp_c ) ) * ( 1.0 + pressure_pHa * ( 4.5e-6 + 6e-10 * temp_c * temp_c )  )
        pw = rel_humidity * ps / ( 1.0 - (1.0-rel_humidity) * ps / pressure_pHa )
    ...
    gamma = 0
    if optic:
      wlsq = wavelength_micrometers * wavelength_micrometers
      gamma = ( ( 77.53484e-6 +
                 ( 4.39108e-7 + 3.666e-9/wlsq ) / wlsq ) * pressure_pHa
                    - 11.2684e-6 * pw ) / temp_k
    else:
       gamma = ( 77.6890e-6*p - ( 6.3938e-6 - 0.375463 / temp_k ) * pw ) / temp_k

    # Formula for beta from Stone, with empirical adjustments
    beta = 4.4474e-6 * temp_k
